...
async def give_filter(client, message):
    if getattr(temp, "MAINTENANCE", False) and (not message.from_user or message.from_user.id not in ADMINS):
        return await message.reply_text("🛠️ Bot maintenance එකක් සිදුවෙමින් පවතී. කරුණාකර ටික වේලාවක් රැඳී සිටින්න.")
    if EMOJI_MODE:
        try:
        ...
        if pm_search:
            ...
            await message.reply_text(
                text=(
                    ...
                    "<blockquote>"
                    "ඔබට 𝑴𝒐𝒗𝒊𝒆 𝑮𝒓𝒐𝒖𝒑 තුළ පමණක් 𝑴𝒐𝒗𝒊𝒆 𝑺𝒆𝒂𝒓𝒄𝒉 කළ හැක. "
                    "𝑫𝒊𝒓𝒆𝒄𝒕 𝑩𝒐𝒕 තුළ 𝑴𝒐𝒗𝒊𝒆 𝑺𝒆𝒂𝒓𝒄𝒉 කිරීමට අවසර නැත. පහත ඇති 𝑹𝑬𝑸𝑼𝑬𝑺𝑻 𝑯𝑬𝑹𝑬 𝑩𝒖𝒕𝒕𝒐𝒏 එක click කර අපගේ 𝑴𝒐𝒗𝒊𝒆 𝑮𝒓𝒐𝒖𝒑 එකට 𝑱𝒐𝒊𝒏 වී ඔබ කැමති 𝑴𝒐𝒗𝒊𝒆 𝑺𝒆𝒂𝒓𝒄𝒉 කරන්න."
                    "</blockquote></b>"
...
async def next_page(bot, query):
    ...
    if not await db.has_premium_access(query.from_user.id):
        return await query.answer("Pagination premium users සඳහා පමණයි.", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
...
async def filter_qualities_cb_handler(client: Client, query: CallbackQuery):
    ...
    if not await db.has_premium_access(query.from_user.id):
        files, offset, total_results = files[:7], "", min(total_results, 7)
    if not files:
    ...
    if offset != "" and await db.has_premium_access(query.from_user.id):
        try:
            if settings['max_btn']:
                btn.append(
...
async def filter_seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    if not await db.has_premium_access(query.from_user.id):
        files, n_offset, total_results = files[:7], "", min(total_results, 7)
    if not files:
        BUTTONS[key] = None
    ...
    if n_offset != "" and await db.has_premium_access(query.from_user.id):
        try:
            if settings['max_btn']:
                btn.append(
                    [InlineKeyboardButton("ᴘᴀɢᴇ", callback_data="pages"), InlineKeyboardButton(
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("request_movie_addon#"):
        title = query.data.split("#", 1)[1].replace("+", " ")
        text = f"<b>Movie Request</b>\n\n<b>Title:</b> {title}\n<b>User ID:</b> <code>{query.from_user.id}</code>"
        for admin in ADMINS:
            try:
                await client.send_message(admin, text)
            except Exception:
                pass
        return await query.answer("Request sent.", show_alert=True)
    ...
    if query.data.startswith("file"):
        ...
        if not await db.has_premium_access(clicked):
            return await query.answer("Send all is available for Premium users only.", show_alert=True)
        ...
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
            content = query.message.text or query.message.caption or ""
            await (query.message.edit_caption(f"<b><strike>{content}</strike></b>") if query.message.photo else query.message.edit_text(f"<b><strike>{content}</strike></b>"))
            await query.message.edit_reply_markup(reply_markup)
            await query.answer("Sᴇᴛ ᴛᴏ Uɴᴀᴠᴀɪʟᴀʙʟᴇ !")
            content = extract_request_content(query.message.text or query.message.caption or "")
            try:
                await client.send_message(
        ...
        if query.from_user.id in ADMINS:
            ...
            await (query.message.edit_caption(f"<b><strike>{content}</strike></b>") if query.message.photo else query.message.edit_text(f"<b><strike>{content}</strike></b>"))
            await query.message.edit_reply_markup(reply_markup)
            await query.answer("දැනට ලබා ගත නොහැකි ලෙස සකසා ඇත!")
            content = extract_request_content(query.message.text or query.message.caption or "")
            try:
                await client.send_message(
                    chat_id=int(from_user),
                    text=(
                        f"<b>Hᴇʏ {user.mention}\n\n"
                        f"ඔබගේ <code>{content}</code> ᴍᴏᴠɪᴇ request එක දැනට ලබා ගත නොහැක. එබැවින් moderatorsට එය upload කළ නොහැක.\n\n"
                ...
                await client.send_message(
                    ...
                    text=(
                        f"<u>Hᴇʏ {user.mention}</u>\n\n"
                        f"<b><code>{content}</code> ᴍᴏᴠɪᴇ එක දැනට ලබා ගත නොහැක. එබැවින් moderatorsට එය upload කළ නොහැක.\n\n"
        ...
        if int(query.from_user.id) == int(from_user):
            user = await client.get_users(from_user)
            await query.answer(
                f"Hᴇʏ {user.first_name}, මෙම ᴍᴏᴠɪᴇ එක දැනට ලබා ගත නොහැක ❌",
                show_alert=True
            )
        else:
        ...
        buttons = [[
            InlineKeyboardButton('Source Code 📜', url='https://t.me/UltraX_Bots'),
            InlineKeyboardButton('⇋ ʙᴀᴄᴋ ⇋', callback_data='about')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
...
async def auto_filter(client, msg, spoll=False):
    ...
    try:
        ...
        if offset != "" and await db.has_premium_access(message.from_user.id):
...
@Client.on_callback_query(filters.regex(r"^legacy_request_movie_addon#"))
async def request_movie_addon_callback(client, query):
    title = query.data.split("#", 1)[1].replace("+", " ")
    text = f"<b>Movie Request</b>\n\n<b>Title:</b> {title}\n<b>User ID:</b> <code>{query.from_user.id}</code>"
    for admin in ADMINS:
        try:
            ...
            pass
    await query.answer("Request sent.", show_alert=True)
    ...
    if not movies:
        ...
        button = [[InlineKeyboardButton(
            "🔍 ᴄʜᴇᴄᴋ sᴘᴇʟʟɪɴɢ ᴏɴ ɢᴏᴏɢʟᴇ 🔍", url=f"https://www.google.com/search?q={google}")],
                  [InlineKeyboardButton("📝 Request movie", url=f"https://t.me/{temp.U_NAME}?start=request_{quote_plus(search)}")]]
