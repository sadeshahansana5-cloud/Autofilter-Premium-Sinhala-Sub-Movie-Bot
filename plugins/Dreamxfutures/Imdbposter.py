...
async def get_tmdb_details_direct(query, id=False, file=None):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            if id and str(query).startswith("tt"):
                ...
                item = next((x for x in data.get("results", []) if x.get("media_type") in ("movie", "tv")), None)
            if not item:
                return None
...
async def get_movie_detailsx(query, id=False, file=None):
    direct = await get_tmdb_details_direct(query, id=id, file=file)
    if direct:
        return direct
