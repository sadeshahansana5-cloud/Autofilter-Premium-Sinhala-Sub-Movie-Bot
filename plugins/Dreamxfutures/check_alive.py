...
ADMIN_ONLY_COMMANDS = {"stats", "grp_broadcast", "send", "pm_search", "movie_update", "verify", "verification", "premium_users", "get_premium", "add_premium", "remove_premium", "delete", "deletefiles", "del_msg", "users", "chats", "enable", "disable", "leave", "ban", "unban", "logs", "restart", "reset_group", "admin_cmd", "limitations", "commands", "broadcast"}

async def register_scoped_commands(client):
    user_commands = [BotCommand(cmd, desc) for cmd, desc in Bot_cmds.items() if cmd not in ADMIN_ONLY_COMMANDS]
    admin_commands = [BotCommand(cmd, desc) for cmd, desc in Bot_cmds.items()]
    await client.set_bot_commands(user_commands)
    for admin_id in ADMINS:
        try:
            await client.set_bot_commands(admin_commands, scope=BotCommandScopeChat(chat_id=int(admin_id)))
        except Exception:
            pass

@Client.on_message(filters.command("commands") & filters.user(ADMINS))
async def set_commands(client, message):
    await register_scoped_commands(client)
