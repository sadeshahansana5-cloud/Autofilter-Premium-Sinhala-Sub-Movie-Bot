...
from info import *
from utils import get_settings, save_group_settings, is_subscribed, is_req_subscribed, get_size, get_shortlink, is_check_admin, temp, get_readable_time, get_time, generate_settings_text, log_error, clean_filename, consume_daily_quota
import time
...
async def start(client, message):
    ...
    if data.startswith("allfiles"):
        quota_ok, quota_used, quota_limit = await consume_daily_quota(message.from_user.id, len(temp.GETALL.get(file_id) or []))
        if not quota_ok:
            return await message.reply_text(f"Daily limit reached: {quota_used}/{quota_limit}")
        try:
    ...
    quota_ok, quota_used, quota_limit = await consume_daily_quota(message.from_user.id)
    if not quota_ok:
        return await message.reply_text(f"Daily limit reached: {quota_used}/{quota_limit}")
    files = files_[0]
