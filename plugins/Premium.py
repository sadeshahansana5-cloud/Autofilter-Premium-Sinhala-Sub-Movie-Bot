...
async def remove_premium(client, message):
    # Additive bulk option; the original single-user behavior below is unchanged.
    if len(message.command) == 2 and message.command[1].lower() == "all":
        result = await db.users.update_many({}, {"$set": {"expiry_time": None}})
        return await message.reply_text(f"Bulk Premium remove complete: {result.modified_count} users")
...
async def give_premium_cmd_handler(client, message):
    # Additive bulk option; the original single-user behavior below is unchanged.
    if len(message.command) == 4 and message.command[1].lower() == "all":
        seconds = await get_seconds(message.command[2] + " " + message.command[3])
        if seconds <= 0:
            return await message.reply_text("Invalid time format")
        expiry_time = datetime.datetime.now() + datetime.timedelta(seconds=seconds)
        result = await db.users.update_many({}, {"$set": {"expiry_time": expiry_time}})
        return await message.reply_text(f"Bulk Premium add complete: {result.modified_count} users")
