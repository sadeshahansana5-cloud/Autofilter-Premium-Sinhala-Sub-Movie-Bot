...
def clean_filename(file_name):
    ...
    # Remove the requested subtitle-channel fragments even when punctuation is attached.
    requested_fragments = (
        "@sinhalasubtitles_rezoth", "sinhalasubtitles_rezoth",
        "sinhalasubtitles", "rezoth",
        "@irupc.net_www_tamilrockers_", "@irupc.net",
        "www_tamilrockers_", "[irupc.net_]", ".net", "net", "irupc",
    )
    file_name = re.sub(r"\(?@?sinhalasubtitles\s+rezoth\)?", "", file_name, flags=re.IGNORECASE)
    for fragment in requested_fragments:
        file_name = re.sub(re.escape(fragment), "", file_name, flags=re.IGNORECASE)
    file_name = re.sub(r"(?<!\w)_+(?!\w)", " ", file_name)
...
# Additive daily quota helper; all original utility functions above remain unchanged.
async def consume_daily_quota(user_id, amount=1):
    import datetime as _datetime
    today = _datetime.datetime.now(_datetime.timezone.utc).strftime("%Y-%m-%d")
    user = await db.users.find_one({"id": int(user_id)}) or {}
    usage = user.get("addon_daily_downloads", {})
    used = int(usage.get("count", 0)) if usage.get("date") == today else 0
    premium = await db.has_premium_access(int(user_id))
    limit = int(await db.get_bot_setting(0, "PREMIUM_DAILY_LIMIT" if premium else "FREE_DAILY_LIMIT", 50 if premium else 5))
    if used + int(amount) > limit:
        return False, used, limit
    await db.users.update_one({"id": int(user_id)}, {"$set": {"addon_daily_downloads": {"date": today, "count": used + int(amount)}}}, upsert=True)
