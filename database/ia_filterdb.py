...
async def get_search_results(chat_id, query, file_type=None, max_results=None, offset=0, filter=False, include_subtitle_files=False, subtitle_only=False):
    ...
    # Normal movie search hides subtitle/archive sidecar files; /sub can request only them.
    subtitle_pattern = re.compile(r"(?:\.|\s)(?:srt|zip|rar|7z|7zip|ass|ssa|sub|vtt)(?:$|\s)", re.IGNORECASE)
    if subtitle_only:
        filter_mongo = {"$and": [filter_mongo, {"file_name": subtitle_pattern}]}
    elif not include_subtitle_files:
        filter_mongo = {"$and": [filter_mongo, {"file_name": {"$not": subtitle_pattern}}]}
